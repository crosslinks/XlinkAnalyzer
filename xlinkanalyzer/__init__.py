...
import random
import string
from itertools import product, groupby, tee, izip, combinations_with_replacement
...
class XlinkDataMgr(DataMgr):
        ...
        for xlinkSet, xlinkDataSet in zip(xlinkSetsCopies, xlinkDataSets):
            for xlink in xlinkSet.data:
                if xlink['Protein1'] in xlinkDataSet:
                    compName = xlinkDataSet[xlink['Protein1']]
                    if compName:
                        xlink['Protein1'] = compName
                if xlink['Protein2'] in xlinkDataSet:
                    compName = xlinkDataSet[xlink['Protein2']]
                    if compName:
    ...
    def color_xlinked(self, to=None, fromComp=None, minLdScore=None, color=None, colorByCompTo=False, uncolorOthers=False):
        ...
        base_color = chimera.colorTable.getColorByName('gray')
    ...
    # def color_xlinked(self, to=None, minLdScore=None):
    #     '''
    #     by_component - color by color of a component it crosslinks to
    #     '''
    #     self.model.colorAll()
    #     base_color = chimera.MaterialColor(255,0,0)
    #     if to is not None:
    #         base_color = chimera.colorTable.getColorByName('gray')
    #         to_color = self.model.config.getColor(to)
    #     else:
    #         to_color = base_color

    #     #first color all xlinking resi with base color
    #     self.color_visible_xlinking_resi(base_color)
    ...
    #         if to is not None:
    #             component = self.model.config.get_chain_for_chimera_obj(obj)]

    #             # xlinked_components = list(data_interface.get_xlinked_components(f))
    #             xlinked_components = [f['Protein1'], f['Protein2']] #xlinks return by self.iter_obj_xlinks are renamed to component names already
    #             index = xlinked_components.index(component)
    #             del xlinked_components[index]
    #             if xlinked_components[0] != to:
    #                 continue
